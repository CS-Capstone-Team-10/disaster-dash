        ...
        {items.map(t => (
          <li key={t.id} className="p-3 rounded-xl border border-gray-800/50 bg-gray-950/50">
            <div className="flex items-center justify-between">
              <div className="font-medium text-sm">{t.handle}</div>
              <div className={`text-[10px] px-2 py-0.5 rounded border ${colorByType[t.disaster]}`}>
                {t.disaster}
              </div>
            </div>
            <p className="text-sm mt-1 text-gray-200">{t.text}</p>
            <div className="mt-2 text-[11px] text-gray-400 suppressHydrationWarning">
