...
export function AvgMttrCard({ valueHours, segments, targetHours, delay = 0 }: AvgMttrCardProps) {
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      className="rounded-2xl border border-border/40 bg-card/50 backdrop-blur-sm p-6 shadow-lg"
      tabIndex={0}
      role="article"
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-medium text-muted-foreground">Average MTTR</h3>
        </div>
      </div>
      ...
      <div className="mt-4 flex items-center justify-center gap-4 text-xs">
        ...
        {targetHours !== undefined && (
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-muted-foreground">Target: {targetHours}h</span>
          </div>
        )}
      </div>
    </motion.div>
