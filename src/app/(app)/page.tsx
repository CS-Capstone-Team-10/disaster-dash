        ...
        >
          <div className="rounded-2xl border border-border/40 bg-card/50 p-4">
            <div className="flex items-center gap-3 mb-3">
              <h2 className="font-semibold text-foreground">US Disaster Heatmap</h2>
              <select
                value={disaster}
                onChange={(e) => setDisaster(e.target.value as Disaster)}
                className="border rounded px-2 py-1 text-sm bg-muted border-border text-foreground"
              >
              ...
              <span className="text-xs text-muted-foreground">{m.windowLabel}</span>
            </div>
            <UsStateChoropleth
              data={stateAgg}
              disaster={disaster}
